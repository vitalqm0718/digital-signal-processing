%% 等波纹最佳逼近法设计 FIR 带通滤波器并滤波
function filtered_signal = equirippleBandPass(music, fs)
% 输入:
%   music - 原始音频信号，行向量
%   fs    - 采样频率，标量
% 输出:
%   filtered_signal - 带通滤波后的音频信号
%% 带通滤波器参数设置
Left_Resistance_Band_Cutoff_Frequency = 300;
% 左阻带截止频率（Hz）
Left_Passband_cutoff_frequency = 320;
% 左通带截止频率（Hz）
Right_Passband_cutoff_frequency = 405;
% 右通带截止频率（Hz）
Right_Resistance_Band_Cutoff_Frequency = 425;
% 右阻带截止频率（Hz）
...
% 采样频率
Vector_of_cutoff_frequency = [Left_Resistance_Band_Cutoff_Frequency, Left_Passband_cutoff_frequency, Right_Passband_cutoff_frequency, Right_Resistance_Band_Cutoff_Frequency];
% 四个截止频率向量
ideal_response = [0, 1, 0];
% 理想响应：左阻带0，通带1，右阻带0
%% 滤波器设计
Maximum_error = [10^(-Minimum_attenuation/20), (10^(Maximum_passband_attenuation/20)-1)/(10^(Maximum_passband_attenuation/20)+1), 10^(-Minimum_attenuation/20)];
% 左阻带、通带、右阻带的最大误差向量
...
title('Time Domain of Band-Pass Filtered Signal');

%% 等波纹最佳逼近法设计 FIR 高通滤波器并滤波
function filtered_signal = equirippleHighPass(music, fs)
% 输入:
%   music - 原始音频信号，行向量
%   fs    - 采样频率，标量
% 输出:
%   filtered_signal - 高通滤波后的音频信号
%% 高通滤波器参数设置
Resistance_Band_Cutoff_Frequency = 360;
% 阻带截止频率（Hz）
Passband_cutoff_frequency = 380;
% 通带截止频率（Hz）
...
% 采样频率
Vector_of_cutoff_frequency = [Resistance_Band_Cutoff_Frequency, Passband_cutoff_frequency];
% 阻带和通带截止频率向量
ideal_response = [0, 1];
% 理想响应：阻带增益为0，通带增益为1
%% 滤波器设计
Maximum_error = [10^(-Minimum_attenuation/20), (10^(Maximum_passband_attenuation/20)-1)/(10^(Maximum_passband_attenuation/20)+1)];
% 阻带和通带的最大误差向量
...
title('Time Domain of High-Pass Filtered Signal');
